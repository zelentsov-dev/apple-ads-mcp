...
description: Research, audit, and safely operate App Store advertising through apple-ads-mcp. Use for Apple Ads account readiness, app eligibility, inventory, keyword opportunities, reports, bulk keyword workflows, recommendations, and explicitly authorized campaign changes; do not use for App Store Connect metadata or Apple Maps ads.
---
...
6. Only preview a mutation when the user's current request authorizes a change.
7. Compare `OperationImpact` with the request, then apply the same receipt and verify every affected object. Stop if state drifted or any result is unknown.
